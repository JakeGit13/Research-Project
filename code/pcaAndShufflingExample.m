function [results] = pcaAndShufflingExample
...
for ii = 1:length(actors)
    ...
    thisMRWarp = data(ii).mr_warp2D(1:38400,:);
    thisVidWarp = data(ii).vid_warp2D(1:38400,:);
    ...
    partial_data(elementBoundaries(reconstructInd)+1:elementBoundaries(reconstructInd+1),:) = 0*(1+rand(length(partial_data(elementBoundaries(reconstructInd)+1:elementBoundaries(reconstructInd+1))),nFrames)); %0; % Set one modality to 0
    ...
    results(ii).nonShuffledLoadings = origloadings;
    results(ii).nonShuffledReconLoadings = partial_loading;
    ...
    plot(results(ii).nonShuffledLoadings,results(ii).nonShuffledReconLoadings,'.');
    ...
    results(ii).allShuffledOrigLoad = allShuffledOrigLoad;
    results(ii).allShuffledReconLoad = allShuffledReconLoad;
    toc
    ...
    results(ii).unshuffstats = [R p(1) SSE];
    ...
    % Original and reconstructed loadings
    subplot(2,3,2);
    title(['participant ', num2str(agent(ii))])
    plot(results(ii).nonShuffledLoadings,results(ii).nonShuffledReconLoadings,'.');
