...
% Root folder for audio WAVs
audioFolderPath = 'C:\Users\jaker\Research-Project\data\Audio\Raw Audio';
audioData = struct([]);  % 1×maxIdx struct with gaps allowed

% Folder to save processed audio
processedAudioFolderPath = 'C:\Users\jaker\Research-Project\data\Audio\Processed Audio';

% [dataIdx, filename]  — used to map dataIdx (MR/Video) to raw audio
manifest = {
     9,  'sub8_sen_256_6_svtimriMANUAL.wav';
     1,  'sub8_sen_252_18_svtimriMANUAL.wav';   % swapped: idx 1 ← sub8_sen_252_18
     5,  'sub1_sen_252_1_svtimriMANUAL.wav';    % swapped: idx 5 ← sub1_sen_252_1
...
for i = 1:size(manifest,1)
    ...
    audioFilePath = fullfile(audioFolderPath, manifest{i,2});
    ...
    dIdx = manifest{r,1};
    base = erase(manifest{r,2}, '.wav');
    S = load(fullfile(outFolderPath, [base '_features.mat']), 'audioData');  % per-item
    audioData(dIdx) = S.audioData;  % places features at dataIdx

    save(fullfile(projectRoot,'audioFeaturesData.mat'), 'audioData', '-v7.3');

    fprintf('\n[INFO] Saved raw features to: audioFeaturesData.mat\n');
